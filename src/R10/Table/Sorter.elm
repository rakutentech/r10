...
sort : R10.Table.State.State -> List (R10.Table.Config.ColumnConf data msg) -> List data -> List data
...
applySorter : Bool -> R10.Table.Types.Sorter data -> List data -> List data
applySorter isReversed sorter data =
    case sorter of
        R10.Table.Types.None ->
            data

        R10.Table.Types.Increasing sort_ ->
            sort_ data

        R10.Table.Types.Decreasing sort_ ->
            List.reverse (sort_ data)

        R10.Table.Types.IncOrDec sort_ ->
            if isReversed then
                List.reverse (sort_ data)
        ...
        R10.Table.Types.DecOrInc sort_ ->
            if isReversed then
                sort_ data
...
findSorter : String -> List (R10.Table.Config.ColumnConf data msg) -> Maybe (R10.Table.Types.Sorter data)
...
-}
unsortable : R10.Table.Types.Sorter data
unsortable =
    R10.Table.Types.None
...
want a table of people, sorted alphabetically by name, we would say this:
    ...
    sorter : R10.Table.State.Sorter { a | name : comparable }
    sorter =
        increasingBy .name

-}
increasingBy : (data -> comparable) -> R10.Table.Types.Sorter data
increasingBy toComparable =
    R10.Table.Types.Increasing (List.sortBy toComparable)
...
would say this:
    ...
    sorter : R10.Table.State.Sorter { a | population : comparable }
    sorter =
        decreasingBy .population

-}
decreasingBy : (data -> comparable) -> R10.Table.Types.Sorter data
decreasingBy toComparable =
    R10.Table.Types.Decreasing (List.sortBy toComparable)
...
This function lets you see both, starting with decreasing order.
    ...
    sorter : R10.Table.State.Sorter { a | sugar : comparable }
    sorter =
        decreasingOrIncreasingBy .sugar

-}
decreasingOrIncreasingBy : (data -> comparable) -> R10.Table.Types.Sorter data
decreasingOrIncreasingBy toComparable =
    R10.Table.Types.DecOrInc (List.sortBy toComparable)
...
sort by best time by default, but also see the other order.
    ...
    sorter : R10.Table.State.Sorter { a | time : comparable }
    sorter =
        increasingOrDecreasingBy .time

-}
increasingOrDecreasingBy : (data -> comparable) -> R10.Table.Types.Sorter data
increasingOrDecreasingBy toComparable =
    R10.Table.Types.IncOrDec (List.sortBy toComparable)
